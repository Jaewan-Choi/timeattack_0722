...
class ApplyView(APIView):
    ...
    def get(self, request):
        status_name = request.query_params.getlist('status', '')[0]
        status_obj = ApplicationStatus.objects.get(status=status_name)
        searches = JobPostActivity.objects.filter(status=status_obj.id)
        return Response(searches.values(), status=status.HTTP_200_OK)
